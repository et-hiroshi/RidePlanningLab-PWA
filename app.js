...
import {APP_VERSION, CALCULATION_CONTRACT_VERSION, appendExecutionSnapshot, createExecutionSnapshot, deleteAllExecutionSnapshots, executionSnapshotsFilename, executionSnapshotsJsonl, loadExecutionSnapshots, sameExecutionSnapshotContent} from './execution_snapshots.js?v=ride-planning-ui-v14';
...
function selected(form){const data=new FormData(form);return [...form.querySelectorAll('[name=selected_event]:checked')].map((box,route_order)=>{const minutes=Number(data.get(`event_${box.value}`));if(!Number.isInteger(minutes)||minutes<0)throw new Error('予定イベント時間は0以上の整数で入力してください。');return {event_code:box.value,route_order,planned_duration_sec:minutes*60,minutes}})}
...
document.querySelector('#destination-form').addEventListener('submit',event=>{event.preventDefault();const node=document.querySelector('#destination-result');currentCalculations.destination=null;try{const form=event.currentTarget,d=Number(form.elements.distance_km.value),time=form.elements.departure_time.value,e=epoch(time),events=selected(form),reserveMinutes=unexpected(form),r=estimateDestination({distance_km:d,departure_epoch_sec:e,event_minutes:events.map(item=>item.minutes),unexpected_buffer_minutes:reserveMinutes},artifact);currentCalculations.destination={calculation:{mode:'distance_to_time',input:{distance_km:d,departure_epoch_sec:e,planned_events:events.map(({minutes,...item})=>item),reserve_time_sec:reserveMinutes*60},result:{standard_elapsed_sec:r.elapsed_time_sec,lower_elapsed_sec:r.elapsed_lower_sec,upper_elapsed_sec:r.elapsed_upper_sec,standard_distance_km:r.distance_km,moving_time_sec:r.moving_time_sec,natural_stop_time_sec:r.natural_stop_time_sec,planned_event_time_sec:r.planned_event_time_sec,reserve_time_sec:r.unexpected_buffer_sec,standard_arrival_epoch_sec:r.arrival_at,lower_arrival_epoch_sec:r.arrival_lower_at,upper_arrival_epoch_sec:r.arrival_upper_at}}};reveal(node,directResult(r,{departure_time:time,epoch:e}))}catch(e){reveal(node,`<div class="error">${esc(e.message)}</div>`)}});
document.querySelector('#distance-form').addEventListener('submit',event=>{event.preventDefault();const node=document.querySelector('#distance-result');currentCalculations.distance=null;try{const form=event.currentTarget,s=form.elements.departure_time.value,d=form.elements.deadline_time.value,start=epoch(s),end=epoch(d);if(end<=start)throw new Error('帰宅期限は出発時刻より後にしてください。');const events=selected(form),reserveMinutes=unexpected(form),r=estimateDistance({departure_epoch_sec:start,deadline_epoch_sec:end,event_minutes:events.map(item=>item.minutes),unexpected_buffer_minutes:reserveMinutes},artifact);currentCalculations.distance={calculation:{mode:'time_to_distance',input:{departure_epoch_sec:start,return_deadline_epoch_sec:end,available_time_sec:r.available_time_sec,planned_events:events.map(({minutes,...item})=>item),reserve_time_sec:reserveMinutes*60},result:{standard_elapsed_sec:r.elapsed_time_sec,standard_distance_km:r.prototype_max_distance_km,lower_distance_km:r.distance_lower_km,upper_distance_km:r.distance_upper_km,moving_time_sec:r.moving_time_sec,natural_stop_time_sec:r.natural_stop_time_sec,planned_event_time_sec:r.planned_event_time_sec,reserve_time_sec:r.unexpected_buffer_sec}}};reveal(node,distanceResult(r,{departure_time:s,deadline_time:d}))}catch(e){reveal(node,`<div class="error">${esc(e.message)}</div>`)}});
function reproduction(){if(!artifactSha256)throw new Error('予測データの確認完了後に保存してください。');return {app_version:APP_VERSION,runtime_artifact_id:artifact.source_identity.operational_release_id,runtime_artifact_sha256:artifactSha256,teacher_version:artifact.source_identity.operational_release_id,teacher_contract_version:artifact.source_identity.teacher_contract,calculation_contract_version:CALCULATION_CONTRACT_VERSION}}
function updateSnapshotCount(message=''){const count=document.querySelector('#snapshot-count'),status=document.querySelector('#snapshot-management-status');try{const records=loadExecutionSnapshots();count.textContent=`${records.length}件`;status.textContent=message}catch(error){count.textContent='読込エラー';status.textContent=error.message}}
document.addEventListener('click',event=>{const button=event.target.closest('[data-save-snapshot]');if(!button)return;const context=currentCalculations[button.dataset.saveSnapshot],panel=button.closest('.execution-save'),status=panel.querySelector('.snapshot-status');if(!context){status.textContent='先に計算してください。';return}button.disabled=true;try{const displayName=panel.querySelector('[data-snapshot-name]').value.trim(),payload={display_name:displayName,calculation:context.calculation,reproduction:reproduction()},records=loadExecutionSnapshots(),latest=records[records.length-1];if(latest&&sameExecutionSnapshotContent(latest,payload)&&displayName===(latest.display_name||'')){status.textContent='変更はありません。';return}const record=createExecutionSnapshot(payload);appendExecutionSnapshot(record);status.textContent='実行用計画を保存しました。';updateSnapshotCount();}catch(error){status.textContent=`保存できませんでした: ${error.message}`}finally{button.disabled=false}});
document.querySelector('#export-snapshots').addEventListener('click',()=>{const status=document.querySelector('#snapshot-management-status');try{const records=loadExecutionSnapshots();if(!records.length){status.textContent='保存済みの実行用計画がありません。';return}const url=URL.createObjectURL(new Blob([executionSnapshotsJsonl(records)],{type:'application/x-ndjson;charset=utf-8'})),link=document.createElement('a');link.href=url;link.download=executionSnapshotsFilename();link.click();URL.revokeObjectURL(url);status.textContent=`${records.length}件をJSONLで出力しました。`}catch(error){status.textContent=`出力できませんでした: ${error.message}`}});
document.querySelector('#delete-snapshots').addEventListener('click',()=>{if(!confirm('保存済みの実行用計画をすべて削除します。元に戻せません。よろしいですか？'))return;const status=document.querySelector('#snapshot-management-status');try{deleteAllExecutionSnapshots();updateSnapshotCount('保存済みの実行用計画をすべて削除しました。')}catch(error){status.textContent=`削除できませんでした: ${error.message}`}});
updateSnapshotCount();
...
async function loadArtifact(){
  ...
  try{const response=await fetchWithin('./artifacts/ride_planning_runtime_v1.json','予測データ');if(!response.ok)throw new Error(`予測データ取得失敗 (${response.status})`);
    ...
    try{const digest=await sha256(bytes);artifactSha256=digest;setText('#artifact-sha',short(digest));return digest}catch(error){artifactSha256=undefined;setText('#artifact-sha',error.message.includes('非対応')?'非対応':'取得失敗');return null}
