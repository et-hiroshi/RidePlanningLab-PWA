...
import {APP_VERSION, CALCULATION_CONTRACT_VERSION, appendExecutionSnapshot, createExecutionSnapshot, deleteAllExecutionSnapshots, executionSnapshotsFilename, executionSnapshotsJsonl, loadExecutionSnapshots} from './execution_snapshots.js';
...
document.addEventListener('click',event=>{const button=event.target.closest('[data-save-snapshot]');if(!button)return;const context=currentCalculations[button.dataset.saveSnapshot],panel=button.closest('.execution-save'),status=panel.querySelector('.snapshot-status');if(!context){status.textContent='先に計算してください。';return}button.disabled=true;try{const record=createExecutionSnapshot({display_name:panel.querySelector('[data-snapshot-name]').value,calculation:context.calculation,reproduction:reproduction()});appendExecutionSnapshot(record);status.textContent='実行用計画を保存しました。';updateSnapshotCount();}catch(error){status.textContent=`保存できませんでした: ${error.message}`}finally{button.disabled=false}});
