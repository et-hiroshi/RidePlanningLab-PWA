...
function directResult(r,input){return `<section class="result"><h1>計算結果</h1><div class="primary-result"><span>標準予測</span><strong>${duration(r.elapsed_time_sec)}</strong><b>${clock(input.epoch,r.arrival_at)} 帰宅</b></div><div class="supporting-range"><span>早め～遅めの目安</span><strong>${duration(r.elapsed_lower_sec)} 〜 ${duration(r.elapsed_upper_sec)}</strong><small>${clock(input.epoch,r.arrival_lower_at)} 〜 ${clock(input.epoch,r.arrival_upper_at)}</small></div><p class="departure">出発 ${esc(input.departure_time)}</p><h2>所要時間の内訳（標準予測）</h2>${allocation(r.moving_time_sec,r.natural_stop_time_sec,r.unexpected_buffer_sec,r.planned_event_time_sec)}<div class="breakdown">${component('moving','走','走行時間','過去の走行実績から距離に応じて推定',duration(r.moving_time_sec))}${component('natural','止','通常停止時間','予定・例外停止を教師から除外し、距離帯の履歴中央値から推定',`${duration(r.natural_stop_time_sec)}<br><small>R059の精度比較は未判定</small>`)}${component('unexpected','備','予備時間','ユーザーが選んだ計画用バッファ（ActualやOracleではありません）',duration(r.unexpected_buffer_sec))}${component('planned','予','予定イベント時間','ユーザーが事前入力したイベント時間の合計',duration(r.planned_event_time_sec))}</div><p class="range-note">予測範囲は走行＋通常停止の過去誤差を反映します。予定イベントと予備時間は各予測へ同じだけ加算します。</p>${warning(r.warnings)}</section>`}
function distanceResult(r,input){const movingLow=r.available_time_sec-r.planned_event_time_sec-r.residual_upper_sec,movingHigh=r.available_time_sec-r.planned_event_time_sec-r.residual_lower_sec;return `<section class="result"><h1>計算結果</h1><div class="primary-result"><span>標準予測</span><strong>${km(r.prototype_max_distance_km)}</strong></div><div class="supporting-range"><span>早め～遅めの目安</span><strong>${km(r.distance_lower_km)} 〜 ${km(r.distance_upper_km)}</strong></div><div class="fixed"><div><span>出発時刻</span><br><strong>${esc(input.departure_time)}</strong></div><b>→</b><div><span>帰宅期限</span><br><strong>${esc(input.deadline_time)}</strong></div><div><span>利用可能時間</span><br><strong>${duration(r.available_time_sec)}</strong></div></div><h2>所要時間の内訳（標準予測）</h2>${allocation(r.moving_time_sec,r.natural_stop_time_sec,r.unexpected_buffer_sec,r.planned_event_time_sec)}<div class="breakdown">${component('moving','走','走行時間','予測範囲に応じて走行へ使える時間',`${duration(movingLow)} 〜 ${duration(movingHigh)}<br><i class="central">標準予測 ${duration(r.moving_time_sec)}</i>`)}${component('natural','止','通常停止時間','予定・例外停止を教師から除外し、距離帯の履歴中央値から推定',`${duration(r.natural_stop_time_sec)}<br><small>R059の精度比較は未判定</small>`)}${component('unexpected','備','予備時間','ユーザーが選んだ計画用バッファ（ActualやOracleではありません）',duration(r.unexpected_buffer_sec))}${component('planned','予','予定イベント時間','ユーザーが事前入力したイベント時間の合計',duration(r.planned_event_time_sec))}</div>${warning(r.warnings)}</section>`}
